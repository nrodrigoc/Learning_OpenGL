...
private:
	...
	std::string loadShaderSource(const char* fileName)
	{
		...
		//Vertex
		in_file.open(fileName); //Arquivo onde esta definido o Vertex Shader
	...
	GLuint loadShader(GLenum type, const char* fileName)
	{
		...
		GLuint shader = glCreateShader(type);  //CRIA O VERTEX SHADER NO BACKGROUND
...
public:
		...
		: versionMajor(versionMajor), versionMinor(versionMinor)
	{
		...
		if (geometryFile != "")
		{
			vertexShader = loadShader(GL_GEOMETRY_SHADER, geometryFile);
		}
	...
	void setMat3fv(glm::mat3 value, const char* name, GLboolean transpose = GL_FALSE)
	{
		this->use();
	...
	void setMat4fv(glm::mat4 value, const char* name, GLboolean transpose = GL_FALSE)
	{
		this->use();
