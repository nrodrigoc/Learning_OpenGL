...
vec3 calculateSpecular(Material material, vec3 vs_position, vec3 vs_normal, vec3 lightPos0, vec3 cameraPos)
{
	...
	float specularConstant = pow(max(dot(posToViewDirVec, reflectDirVec), 0), 40); //30 - > Tamanho da specular light
...
void main()
{
	...
	fs_color = 
	texture(material.diffuseTex, vs_texcoord) /* * texture(texture1, vs_texcoord)*/ * vec4(vs_color, 1.f)
